...
describe("<IconComponent/>", () => {
  ...
  describe("classname", () => {
    test("should have proper classname with default props", () => {
      ...
      expect(icon.find("i").prop("className")).toEqual(
        "oauth-login-icon fa fa fa-google"
      )
    })
    test("should show loading", () => {
      ...
      expect(icon.find("i").length).toEqual(1)
      expect(icon.find("i").prop("className")).toEqual(
        expect.stringContaining("fa  fa-circle-o-notch fa-spin")
      )
    })
    test("should show special span", () => {
      ...
      expect(icon.find("i").length).toEqual(1)
      expect(icon.find("i").prop("className")).toEqual(
        expect.stringContaining("fa-spin")
      )
      expect(icon.find("i").prop("className")).toEqual(
        expect.stringContaining(" fa  spinner")
      )
    })
    test("should not show special span if isLoading is false", () => {
      ...
      expect(icon.find("i").length).toEqual(1)
      expect(icon.find("i").prop("className")).toEqual(
        expect.not.stringContaining("fa-spin")
      )
      expect(icon.find("i").prop("className")).toEqual(
        expect.not.stringContaining(" fa  spinner")
      )
    })
    test("should show prop size", () => {
      ...
      expect(icon.find("i").length).toEqual(1)
      expect(icon.find("i").prop("className")).toEqual(
        expect.stringContaining("oauth-login-icon-sm")
      )
    })
